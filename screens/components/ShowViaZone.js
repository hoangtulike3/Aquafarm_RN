...
export default ShowViaZone = (props) => {
  const { t } = useTranslation();
  const values = props.value;
  const navigation = props.navigation;
  const zone = props.zone;
  const [valuesViaZone, setValuesViaZone] = useState([])

  const getValuesViaZone = (listItem) => {
    let newList = [];
    listItem.forEach(element => {
      if(element.deviceInfor.farm_name == zone)
      {
        newList.push(element);
      }
    });
    setValuesViaZone(newList);
  }
  ...
  useEffect(() => {
    getValuesViaZone(values)
  },[props]);

  return (
    valuesViaZone?.map( (value, index) =>
