...
import { StyleSheet, Text, Alert, View, Dimensions, TouchableOpacity, Image} from 'react-native';
import { Buffer } from "buffer";

export default ShowNoZone = (props) => {
  const { t } = useTranslation();
  const values = props.value;
  const navigation = props.navigation;
  const [valuesNoZone, setValuesNoZone] = useState([])

  const getValuesNoZone = (listItem) => {
    let newList = [];
    listItem.forEach(element => {
      if(element.deviceInfor.farm_name == null || element.deviceInfor.farm_name == "null")
      {
        newList.push(element);
      }
    });
    setValuesNoZone(newList);
  }
  ...
  useEffect(() => {
    getValuesNoZone(values)
  },[props]);

  return (
    valuesNoZone?.map( (value, index) =>
